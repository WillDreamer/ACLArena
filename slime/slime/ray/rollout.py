...
def start_rollout_servers(args, pg) -> dict[str, RolloutServer]:
    ...
    for model_idx, model_cfg in enumerate(config.models):
        ...
        if all_init_handles:
            # Bound the wait on SGLang engine init. Each engine's init() does its own
            # intra-node TP NCCL setup + weight load + CUDA-graph capture; if any engine
            # wedges there (observed: TP NCCL init hangs mid-channel-setup on a bad node
            # draw), an unbounded ray.get() waits FOREVER and the whole job silently burns
            # GPUs. Fail loud after a timeout so the job dies fast and can be retried onto
            # a fresh node set. Override via SLIME_ROLLOUT_ENGINE_INIT_TIMEOUT_S (default
            # 1800s = 30min, generous for 8B weight load + cudagraph on a slow draw).
            _init_timeout = int(os.environ.get("SLIME_ROLLOUT_ENGINE_INIT_TIMEOUT_S", "1800"))
            _GetTimeoutError = getattr(ray.exceptions, "GetTimeoutError", TimeoutError)
            logger.info(
                f"Waiting up to {_init_timeout}s for {len(all_init_handles)} SGLang "
                f"engine(s) of model '{model_cfg.name}' to finish init()."
            )
            try:
                ray.get(all_init_handles, timeout=_init_timeout)
            except (_GetTimeoutError, TimeoutError) as e:
                raise RuntimeError(
                    f"SGLang engine init for model '{model_cfg.name}' did not complete "
                    f"within {_init_timeout}s: at least one of {len(all_init_handles)} "
                    f"engine(s) is stuck in init() (typically the intra-node TP NCCL "
                    f"rendezvous or weight load/CUDA-graph capture). Retry the job (often "
                    f"lands a healthier node set) or raise "
                    f"SLIME_ROLLOUT_ENGINE_INIT_TIMEOUT_S."
                ) from e
