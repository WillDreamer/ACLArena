...
def all_gather_param(name: str, param: torch.nn.Parameter) -> torch.Tensor:
    ...
    # tp_size == 1: nothing to gather across ranks. The local param IS the full param, so the
    # concat/GLU-rechunk below is a no-op and partition_stride is irrelevant (there is no
    # cross-rank interleaving to undo). Short-circuit BEFORE the partition_stride assert — that
    # assert is only meaningful when tp_size > 1. Without this, Qwen3-8B's fused-QKV carries
    # partition_stride=3 (set at layer construction, independent of TP world size) and trips the
    # assert even at TP=1, blocking the disaggregated weight-sync (MOPD needs TP=1 there because
    # the distributed path can't de-interleave strided QKV at TP>1). Returning param.data is
    # exactly what the tp_size>1 path would compute for a single partition.
    if tp_size == 1:
        return param.data
...
def all_gather_params_async(
    ...
    for info, param in param_infos_and_params:
        ...
        if "expert_bias" in info.name:
            ...
            # tp_size == 1: no cross-rank gather; local param is the full param (mirrors the
            # sync all_gather_param short-circuit — avoids the spurious partition_stride assert
            # on strided fused-QKV at TP=1).
            if tp_size == 1:
                gather_tasks.append((info, param.data, None, None, None))
                handles.append(None)
                continue
...
def _named_params_and_buffers_vanilla(model: Sequence[torch.nn.Module]) -> Iterator[tuple[str, torch.Tensor]]:
    for vp_stage, model_module in enumerate(model):
        ...
        for name, param in model_module.named_parameters():
            if "lora_A" in name or "lora_B" in name:  # LoRA adapters: never synced to SGLang
                continue
