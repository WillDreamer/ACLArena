...
# The initial actor->rollout weight-sync group is [train rank0 + all engine GPUs]
# formed over a TCP store on train rank0's EPHEMERAL port (see
# connect_rollout_engines_from_distributed). When the disaggregated nodes span
# different /24 subnets, that ephemeral TCP port may be unreachable cross-/24
# (Cluster only guarantees AZ-level topology) and the rendezvous spins forever.
# TORCH_NCCL_TIMEOUT_MS / TORCH_NCCL_BLOCKING_WAIT do NOT cover this: they govern
# NCCL COLLECTIVES, not the pre-NCCL TCPStore rendezvous inside init_process_group.
# So we set an explicit store/PG timeout here AND bound the ray.get on the
# engine-side joins. Override via env (seconds); default 600s = 10 min.
# (See memory slime-disagg-weightsync-crosssubnet-hang.)
...
def connect_rollout_engines_from_distributed(
    ...
    # ray.get(timeout=) raises ray.exceptions.GetTimeoutError; resolve the class
    # defensively so a Ray-version rename can't mask the real diagnosis.
