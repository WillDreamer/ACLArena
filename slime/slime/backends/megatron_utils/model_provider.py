...
def wrap_model_provider_with_freeze(original_provider, args):
    def wrapped_provider(pre_process=True, post_process=True, vp_stage=None, **kwargs):
        # Megatron's build_model() may call the provider with extra kwargs (e.g.
        # `config=`) depending on version. Forward only the kwargs the wrapped
        # provider actually accepts so we don't crash on an unexpected keyword.
