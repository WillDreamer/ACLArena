...
def get_batch(
    ...
    for loss_mask, total_length, response_length in zip(
        ...
        # Guard against prompt_length <= 0 (a degenerate sample whose whole token
        # stream is counted as response, e.g. a chat-template/mask edge case): a
        # negative left-pad makes torch F.pad call narrow() with a negative length
        # and crash the entire train step. Clamp the left pad to >= 0 so one bad
        # sample can't take down the job; the resulting mask is still right-aligned.
        left_pad = max(prompt_length - 1, 0)
        loss_mask = F.pad(loss_mask, (left_pad, 1), value=0)
        if allgather_cp:
