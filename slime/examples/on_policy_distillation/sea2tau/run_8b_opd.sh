...
#   teacher : MultiStageRL/Qwen3-8B-Base-SeaSFT-Search (search specialist) -> local SGLang server
#
...
# The student rolls out with the SEARCH multi-turn generate fn (calls the retriever),
# and the teacher scores token-level logprobs over the student's own trajectory.
#
...
#
# usage: cd /data/user/slime && bash examples/on_policy_distillation/sea2tau/run_8b_opd.sh
...
WANDB_GROUP="opd_search2tau_bs_${ROLLOUT_BATCH_SIZE}"
...
CKPT_ARGS=(
   ...
   # STUDENT is a full slime RL ckpt (iter_0000300, weights+optimizer+LR-scheduler from a
   # gbs=512 run -> lr_decay_steps=153600). This is a *fresh* OPD run (its own schedule:
   # 600 iters * gbs 96 -> 57600), so load weights only: no optimizer/rng/scheduler restore,
   # and reset iteration -> 0 so all --num-rollout rollouts actually run. Without these, the
   # OptimizerParamScheduler assert (57600 != 153600) crashes init, and even past it the run
   # would start at rollout 300 and do nothing.
   --no-load-optim
   --no-load-rng
   --finetune
   --save          ${MODEL_ROOT}/MultiStageRL/OPD/Qwen3-8B-Base-TauSFT-Tau_OPD_Search/
   --save-interval 30
)
