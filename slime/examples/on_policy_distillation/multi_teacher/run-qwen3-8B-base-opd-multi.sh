#!/bin/bash
# 3-teacher Entropy-Aware on-policy distillation (EOPD, pure distillation task reward = 0) —
# CLUSTER / multi-node. Reverse-KL OPD (mode-seeking) gated to LOW-entropy teacher tokens;
# forward-KL (mode-covering) gated to HIGH-entropy teacher tokens; entropy threshold = 0.5
# nats. See GRPO_ARGS + arXiv:2603.07079. Requires --context-parallel-size 1.
# VARIANT: student init = oracle-mix SFT-balance @ iter 500 (light SFT on base; skips the full sequential
# SFT chain but is NOT raw base — raw base couldn't produce trainable agentic rollouts, see run 262202).
#
#   student : Qwen3-8B-oracle-mix-SFT500  (megatron torch_dist, --load)  <-- lightly-SFT'd base, NOT raw base
...
# VARIANT: student init = the oracle-mix SFT-balance checkpoint at iter 500 (Megatron torch_dist),
# NOT the raw base and NOT the end-of-chain Math-SeaSFT-Search-TauSFT-Tau-IF model. Direct-from-base
# OPD failed to learn (eval flat, degenerate rollouts, ~half truncated), because raw base lacks the
# tool-call / instruction-following priors needed to emit trainable agentic trajectories. This uses a
# lightly-SFT'd base (oracle-mix SFT @ step 500) as the student init so rollouts are well-formed enough
# for the 3 domain teachers to distill into. The ckpt is re-laid-out as a Megatron "release" checkpoint
# (weights only, iteration 0, fresh optimizer — same convention as the SFT-chain student) at S3
# ACLArena/Qwen3-8B-oracle-mix-SFT500_torch_dist, staged to $MODEL_ROOT via the CLI --stage-model override.
STUDENT=${MODEL_ROOT}/Qwen3-8B-oracle-mix-SFT500_torch_dist
...
CKPT_ARGS=(
   ...
   --load          ${STUDENT}                                 # STUDENT = oracle-mix SFT@500 (init point, NOT a resume)
   --no-load-optim
   --no-load-rng
   --finetune
   --save          ${MODEL_ROOT}/ACLArena/OPD/Qwen3-8B-oracleSFT500_EOPD_MathSearchTau/
...
# EOPD reward: per-sample teacher logprobs + per-token top-k distribution + per-token
# entropy (domain-routed to teacher_{math,search,tau}), task reward = 0. The scalar
# logprobs drive the reverse-KL OPD advantage path; the top-k + entropy drive the EOPD
# forward-KL term (loss.compute_eopd_forward_kl) that fires on high-entropy teacher tokens.
RM_ARGS=(
   --custom-rm-path opd_multi_teacher.reward_func_eopd
   --custom-reward-post-process-path opd_multi_teacher.post_process_rewards_eopd
)
...
PERF_ARGS=(
   ...
   # partition_stride is trivially 1 and the assert cannot fire — zero-risk.
   #
   # CP=1 (EOPD requirement). --use-eopd-forward-kl hard-asserts context-parallel-size == 1
   # (slime/utils/arguments.py:1695 + backends/megatron_utils/actor.py:280): the 2D teacher top-k
   # tensors are NOT CP zigzag-sliced. Was CP=2 pre-EOPD. Train node = 8 GPU -> TP1 x CP1 x PP1
   # gives DP=8 (fills 8). 8B at TP=1 comfortably fits an H200 (141 GB) — same story as CP=2.
   # --max-tokens-per-gpu bumped to accommodate the un-CP-sharded longest single sequence:
   # search/tau prompt (retrieved wiki up to ~12-14k) + math response up to 8192 ~= 20k tokens.
   ...
   --max-tokens-per-gpu 20480        # >= longest single sequence (CP=1); ~14k prompt + 8192 resp
)

# EOPD (Entropy-Aware OPD, arXiv:2603.07079) on top of GRPO; the only signal is the OPD KL.
# Textbook EOPD entropy split: high-entropy teacher tokens (H > threshold) -> forward-KL
# (mode-covering / preserves diversity where reverse-KL alone collapses); low-entropy tokens
# (H <= threshold) -> reverse-KL (mode-seeking / sharpens the teacher's confident moves).
# --use-opd-entropy-gate makes the reverse-KL term the exact complement of --use-eopd-forward-kl
# so the two never overlap and fight. Requires the EOPD reward fns above so teacher_topk_ids +
# teacher_entropy are produced, and --context-parallel-size 1 (see PERF_ARGS).
GRPO_ARGS=(
   --advantage-estimator grpo
   --use-opd
   --opd-type sglang
   --opd-kl-coef ${OPD_KL_COEF:-0.5}          # reverse-KL, mode-seeking; gated to LOW-entropy tokens
   --use-eopd-forward-kl                       # forward-KL, mode-covering; gated to HIGH-entropy tokens
   --eopd-fkl-coef ${EOPD_FKL_COEF:-0.2}       # forward-KL coef (warmup TARGET; see below)
   --eopd-entropy-threshold ${EOPD_ENTROPY_THRESHOLD:-0.5}  # entropy split point (nats)
   --eopd-topk ${EOPD_TOPK:-16}
   # Warmup: the forward-KL term transiently over-inflates student entropy/length while the
   # model is still far from the teacher (off-equilibrium). Ramp from a small start coef up to
   # the target over the first N rollouts so terminating/low-entropy structure establishes first,
   # then the full mode-covering pressure kicks in. Mirrors the tau2if EOPD schedule.
   --eopd-fkl-warmup-rollouts ${EOPD_FKL_WARMUP_ROLLOUTS:-40}
   --eopd-fkl-warmup-start-coef ${EOPD_FKL_WARMUP_START_COEF:-0.05}
   # Complementary gate on reverse-KL: fire only on LOW-entropy teacher tokens (H <= threshold),
   # the exact complement of the forward-KL high-entropy gate. Threshold defaults to
   # --eopd-entropy-threshold when unset, so the two gates partition the tokens.
...
WANDB_ARGS=(
   --use-wandb
   --wandb-project MOPD
   --wandb-group ${WANDB_GROUP:-eopd_Qwen3-8B-oracleSFT500_math_search_tau}
