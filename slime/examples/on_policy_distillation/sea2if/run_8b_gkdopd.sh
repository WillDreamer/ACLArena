#!/bin/bash
#
# GKD-style On-Policy Distillation (forward-KL ONLY), SEARCH domain, all-local on one 8-GPU box.
...
#     removed, gating would leave low-entropy tokens (answer/format/search-syntax) with zero
...
#
# usage: cd /data/user/slime && bash examples/on_policy_distillation/sea2if/run_8b_gkdopd.sh
...
WANDB_GROUP="gkdopd_search2if_bs_${ROLLOUT_BATCH_SIZE}"
...
CKPT_ARGS=(
   ...
   --no-load-rng
   --finetune
   --save          ${MODEL_ROOT}/MultiStageRL/OPD/Qwen3-8B-Base-TauSFT-Tau-IF_GKD_Search/
   --save-interval 5
