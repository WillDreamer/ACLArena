...
# Encoder device. On Cluster every GPU is allocated to training/rollout, so the e5
# encoder must run on CPU (set RETRIEVER_ENCODER_DEVICE=cpu). Defaults to cuda for the dev box.
ENCODER_DEVICE = os.environ.get("RETRIEVER_ENCODER_DEVICE", "cuda")
...
def load_model(model_path: str, use_fp16: bool = False):
    ...
    if use_fp16 and ENCODER_DEVICE != "cpu":
        # fp16 on CPU is unsupported by most kernels; keep fp32 on CPU.
        model = model.half()
